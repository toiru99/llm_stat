---
type: Model
title: Gemma 4 E4B (Non-reasoning)
creator: Google
license: Open
intelligence_index: 9.0
price_blended_usd_1m: 0.028
output_speed_tps: 43.0
context_window: 128000
status: current
size_class: Small
params_b: 8
is_reasoning: false
radar:
  knowledge: { s: 20.7, z: -0.98, r: 35.4, estimated: false }  # 전문 지식
  reasoning: { s: 19.9, z: -0.73, r: 39.1, estimated: false }  # 추론
  coding: { s: 8.3, z: -1.63, r: 25.6, estimated: false }  # 코딩
  agentic: { s: 19.2, z: -0.7, r: 39.5, estimated: false }  # 에이전트
  trust: { s: 45.4, z: 1.0, r: 65.0, estimated: false }  # 신뢰성
  multimodal: { s: 51.4, z: -0.96, r: 35.6, estimated: false }  # 멀티모달
  long_context: { s: 26.5, z: -0.71, r: 39.3, estimated: false }  # 긴문맥
  instruction: { s: 40.8, z: -0.5, r: 42.5, estimated: false }  # 지시 따르기
sources:
  - type: leaderboard
    title: Artificial Analysis — Gemma 4 E4B (Non-reasoning)
    url: https://artificialanalysis.ai/leaderboards/models
updated: 2026-09-02
timestamp: 2026-09-02T00:00:00Z
---

# Gemma 4 E4B (Non-reasoning)

Google · Open · Small(8B) · 컨텍스트 128k · 종합지능 **9.0**

## 강점 / 약점 (평균 대비)
- **강점**: 신뢰성, 지시 따르기
- **약점**: 전문 지식, 코딩

## 실용 지표
`입력 $0.02 · 출력 $0.1 · 혼합 $0.028/1M · 43.0 t/s · TTFT 0.75s · 128k ctx` · 가성비 321.4

## 레이더 8축 (평균=50 기준선)

| 축 | 점수(r) | 평균대비(z) | 상태 | 구성 벤치마크(raw%) |
|---|---|---|---|---|
| 전문 지식 | 35.4 | -0.98 | 실측 | [[aa-omniscience]] 8.0%×1.0, [[gpqa-diamond]] 55.0%×0.4, [[humanitys-last-exam]] 5.0%×0.3 |
| 추론 | 39.1 | -0.73 | 실측 | [[critpt]] 0.0%×1.0, [[gpqa-diamond]] 55.0%×1.0, [[humanitys-last-exam]] 5.0%×1.0 |
| 코딩 | 25.6 | -1.63 | 실측 | [[scicode]] 4.0%×1.0, [[terminal-bench]] 8.0%×0.5 |
| 에이전트 | 39.5 | -0.7 | 실측 | [[tau2-bench]] 26.0%×1.0, [[terminal-bench]] 8.0%×1.0 |
| 신뢰성 | 65.0 | +1.0 | 실측 | [[aa-omniscience]] 46.0%×1.0 |
| 멀티모달 | 35.6 | -0.96 | 실측 | [[mmmu-pro]] 51.0%×1.0 |
| 긴문맥 | 39.3 | -0.71 | 실측 | [[aa-lcr]] 22.0%×1.0 |
| 지시 따르기 | 42.5 | -0.5 | 실측 | [[ifbench]] 41.0%×1.0 |

> r=50이 추적 모델 평균. 50 초과=평균 이상. '추정'=같은 축 결측을 kNN으로 보완. '측정 안 됨'=미측정(추정 보류).

## 출처
출처: [artificialanalysis.ai](https://artificialanalysis.ai/leaderboards/models)
