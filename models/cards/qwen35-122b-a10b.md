---
type: Model
title: Qwen3.5 122B A10B
creator: Alibaba
license: Open
intelligence_index: 32.0
price_blended_usd_1m: 0.68
output_speed_tps: 142.0
context_window: 262000
radar:
  knowledge: { s: 52.3, z: 0.01, r: 50.1, estimated: false }  # 전문 지식
  reasoning: { s: 45.3, z: -0.0, r: 49.9, estimated: false }  # 추론
  coding: { s: 63.1, z: 0.11, r: 51.7, estimated: false }  # 코딩
  agentic: { s: 53.6, z: 0.09, r: 51.4, estimated: false }  # 에이전트
  trust: { s: 13.8, z: -0.74, r: 39.0, estimated: false }  # 신뢰성
  multimodal: { s: 84.5, z: 0.44, r: 56.7, estimated: false }  # 멀티모달
  long_context: { s: 90.5, z: 0.7, r: 60.5, estimated: false }  # 긴문맥
  instruction: { s: 90.1, z: 0.92, r: 63.9, estimated: false }  # 지시 따르기
sources:
  - type: leaderboard
    title: Artificial Analysis — Qwen3.5 122B A10B
    url: https://artificialanalysis.ai/leaderboards/models
updated: 2026-07-01
timestamp: 2026-07-01T00:00:00Z
---

# Qwen3.5 122B A10B

Alibaba · Open · 컨텍스트 262k · 종합지능 **32.0**

## 강점 / 약점 (평균 대비)
- **강점**: 지시 따르기, 긴문맥
- **약점**: 추론, 신뢰성

## 실용 지표
`입력 $0.4 · 출력 $3.2 · 혼합 $0.68/1M · 142.0 t/s · TTFT 2.4s · 262k ctx` · 가성비 47.1

## 레이더 8축 (평균=50 기준선)

| 축 | 점수(r) | 평균대비(z) | 상태 | 구성 벤치마크(raw%) |
|---|---|---|---|---|
| 전문 지식 | 50.1 | +0.01 | 실측 | [[aa-omniscience]] 25.0%×1.0, [[gpqa-diamond]] 86.0%×0.4, [[humanitys-last-exam]] 23.0%×0.3 |
| 추론 | 49.9 | +-0.0 | 실측 | [[critpt]] 1.0%×1.0, [[gpqa-diamond]] 86.0%×1.0, [[humanitys-last-exam]] 23.0%×1.0 |
| 코딩 | 51.7 | +0.11 | 실측 | [[scicode]] 42.0%×1.0, [[terminal-bench]] 31.0%×0.5 |
| 에이전트 | 51.4 | +0.09 | 실측 | [[gdpval]] 24.0%×1.0, [[tau2-bench]] 94.0%×1.0, [[tau3-banking]] 14.0%×1.0, [[terminal-bench]] 31.0%×1.0 |
| 신뢰성 | 39.0 | -0.74 | 실측 | [[aa-omniscience]] 15.0%×1.0 |
| 멀티모달 | 56.7 | +0.44 | 실측 | [[mmmu-pro]] 75.0%×1.0 |
| 긴문맥 | 60.5 | +0.7 | 실측 | [[aa-lcr]] 67.0%×1.0 |
| 지시 따르기 | 63.9 | +0.92 | 실측 | [[ifbench]] 76.0%×1.0 |

> r=50이 추적 모델 평균. 50 초과=평균 이상. '추정'=같은 축 결측을 kNN으로 보완. '측정 안 됨'=미측정(추정 보류).

## 출처
출처: [artificialanalysis.ai](https://artificialanalysis.ai/leaderboards/models)
