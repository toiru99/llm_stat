---
type: Model
title: Hermes 4 405B (Non-reasoning)
creator: Nous Research
license: Open
intelligence_index: 9.0
price_blended_usd_1m: 1.2
output_speed_tps: 44.0
context_window: 128000
status: current
size_class: Large
params_b: 406
is_reasoning: false
radar:
  knowledge: { s: 35.9, z: -0.23, r: 46.5, estimated: false }  # 전문 지식
  reasoning: { s: 19.0, z: -0.78, r: 38.3, estimated: false }  # 추론
  coding: { s: 42.7, z: -0.18, r: 47.2, estimated: false }  # 코딩
  agentic: { s: 21.2, z: -0.63, r: 40.6, estimated: false }  # 에이전트
  trust: { s: 18.6, z: -0.29, r: 45.7, estimated: false }  # 신뢰성
  multimodal: { s: null, z: null, r: null, estimated: false }  # 멀티모달
  long_context: { s: 25.3, z: -0.76, r: 38.6, estimated: false }  # 긴문맥
  instruction: { s: 32.4, z: -0.86, r: 37.1, estimated: false }  # 지시 따르기
sources:
  - type: leaderboard
    title: Artificial Analysis — Hermes 4 405B (Non-reasoning)
    url: https://artificialanalysis.ai/leaderboards/models
updated: 2026-09-03
timestamp: 2026-09-03T00:00:00Z
---

# Hermes 4 405B (Non-reasoning)

Nous Research · Open · Large(406B) · 컨텍스트 128k · 종합지능 **9.0**

## 강점 / 약점 (평균 대비)
- **강점**: 코딩, 전문 지식
- **약점**: 추론, 지시 따르기

## 실용 지표
`입력 $1.0 · 출력 $3.0 · 혼합 $1.2/1M · 44.0 t/s · TTFT 2.37s · 128k ctx` · 가성비 7.5

## 레이더 8축 (평균=50 기준선)

| 축 | 점수(r) | 평균대비(z) | 상태 | 구성 벤치마크(raw%) |
|---|---|---|---|---|
| 전문 지식 | 46.5 | -0.23 | 실측 | [[aa-omniscience]] 26.0%×1.0, [[gpqa-diamond]] 54.0%×0.4, [[humanitys-last-exam]] 4.0%×0.3 |
| 추론 | 38.3 | -0.78 | 실측 | [[critpt]] 0.0%×1.0, [[gpqa-diamond]] 54.0%×1.0, [[humanitys-last-exam]] 4.0%×1.0 |
| 코딩 | 47.2 | -0.18 | 실측 | [[scicode]] 35.0%×1.0, [[terminal-bench]] 10.0%×0.5 |
| 에이전트 | 40.6 | -0.63 | 실측 | [[tau2-bench]] 27.0%×1.0, [[terminal-bench]] 10.0%×1.0 |
| 신뢰성 | 45.7 | -0.29 | 실측 | [[aa-omniscience]] 20.0%×1.0 |
| 멀티모달 | — | — | 측정 안 됨 | — |
| 긴문맥 | 38.6 | -0.76 | 실측 | [[aa-lcr]] 21.0%×1.0 |
| 지시 따르기 | 37.1 | -0.86 | 실측 | [[ifbench]] 35.0%×1.0 |

> r=50이 추적 모델 평균. 50 초과=평균 이상. '추정'=같은 축 결측을 kNN으로 보완. '측정 안 됨'=미측정(추정 보류).

## 출처
출처: [artificialanalysis.ai](https://artificialanalysis.ai/leaderboards/models)
